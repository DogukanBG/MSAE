...
class Transcoder(nn.Module):
    ...
    def __init__(
        self, n_latents: int, n_inputs: int, n_outputs: int, activation: Callable = nn.ReLU(), tied: bool = False, normalize: bool = False,
        input_bias_init: torch.Tensor | float = 0.0, output_bias_init: torch.Tensor | float = 0.0, init_method: str = "kaiming", latent_soft_cap: float = 30.0, threshold: torch.Tensor | None = None,
        *args, **kwargs
    ) -> None:
        """
        Initialize the sparse autoencoder.
        ...
        # Initialize parameters
        self.pre_bias = nn.Parameter(torch.full((n_inputs,), input_bias_init) if isinstance(input_bias_init, float) else input_bias_init)
        self.post_bias = nn.Parameter(torch.full((n_outputs,), output_bias_init) if isinstance(output_bias_init, float) else output_bias_init)
        ...
        if tied:
            ...
            self.decoder = nn.Parameter(torch.zeros((n_latents, n_outputs)))
            print("DEBUG DECODER:", self.decoder.shape)
        ...
        print(f"DEBUG: n_inputs={n_inputs}, n_outputs={n_outputs}, n_latents={n_latents}")
        print(f"DEBUG: pre_bias shape: {self.pre_bias.shape}")
        print(f"DEBUG: post_bias shape: {self.post_bias.shape}")
        print(f"DEBUG: encoder shape: {self.encoder.shape}")
        print(f"DEBUG: latent_bias shape: {self.latent_bias.shape}")
    ...
    def _init_weights(self, norm=0.1, neuron_indices: list[int] | None = None) -> None:
        ...
        if self.init_method == "kaiming":
            new_W_enc = nn.init.kaiming_uniform_(torch.zeros_like(self.encoder), nonlinearity='relu')
        elif self.init_method == "xavier":
            new_W_enc = nn.init.xavier_uniform_(torch.zeros_like(self.encoder), gain=nn.init.calculate_gain('relu'))
        elif self.init_method == "uniform":
            new_W_enc = nn.init.uniform_(torch.zeros_like(self.encoder), a=-1, b=1)
        elif self.init_method == "normal":
            new_W_enc = nn.init.normal_(torch.zeros_like(self.encoder))
        
        # Normalize encoder columns to target norm
        new_W_enc *= (norm / new_W_enc.norm(p=2, dim=0, keepdim=True))
    ...
    def decode(self, latents: torch.Tensor, info: dict[str, Any] | None = None) -> torch.Tensor:
        ...
        if self.tied:
            ...
            ret = latents @ self.decoder #+ self.post_bias
...
def load_model(path, is_transcoder=False):
    """
    ...
    if not is_transcoder:
        ...
        # /BS/disentanglement/work/msae/768_3072_768_ReLU_003_False_False_0.0_imagenet_dinov2_vitb14_embeddings_train.pth
        ...
        n_latents = int(path_name_spited.pop(0))
        n_inputs = int(path_name_spited.pop(0))
        n_outputs = int(path_name_spited.pop(0))
        ...
        # Create and load the model
        model = Transcoder(n_latents, n_inputs, n_outputs ,activation, tied=tied, normalize=normalize, latent_soft_cap=latent_soft_cap)
        model_state_dict = torch.load(path, map_location='cuda' if torch.cuda.is_available() else 'cpu', weights_only=False)
        model.load_state_dict(model_state_dict['model'])
        mean_center = model_state_dict['mean_center']
        scaling_factor = model_state_dict['scaling_factor']
